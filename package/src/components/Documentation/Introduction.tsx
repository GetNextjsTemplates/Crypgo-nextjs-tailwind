        ...
        <>
         ...
         {docNavbarOpen && (
          <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 z-40" onClick={() => setDocNavbarOpen(false)} />
            ...
            <h3 className=" text-white text-2xl mt-4 font-semibold mb-6" >Pacakge Versions</h3>
            <button onClick={() => setDocNavbarOpen(true)} className="p-0"> <Icon icon="gg:menu-right" className="text-3xl lg:hidden block" /></button>
            </div>
            
              <div className="w-full flex justify-between gap-6 lg:flex-nowrap flex-wrap p-6 rounded-md border border-border dark:border-dark_border">
                {
                    PackageVersions && PackageVersions.map((item)=>{
                        return(
                            ...
                            <p className="text-base font-medium text-muted text-opacity-60">{item.packageName}</p>
                        </div>
                        )
                    })
                }
              ...
              <div className="mt-5">
              <p className="text-base font-medium text-muted text-opacity-60">Crypgo Tailwind NextJs Template is built with Tailwindcss and Nextjs.</p>
